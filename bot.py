...
# Создаем объект бота
bot = telebot.TeleBot('7314417411:AAEjIyb-REt74jg5vVmuxBHL9ro-OCXmbz4')
...
# Выбор количества продукта и добавление в корзину
...
def get_user_loc(msg, text):
    ...
    if msg.location:
        ...
        bot.send_message(-4187935991, text)
        bot.send_location(-4187935991, latitude=msg.location.latitude,
...
def admin_choice(msg):
    ...
    if msg.text == 'Добавить товар':
        ...
        if db.check_pr():
            bot.send_message(admin_id, 'Выберите продукт',
                             reply_markup=bt.admin_pr(db.get_pr_id()))
            # Переход на этап получения аттрибута
...
def get_pr_photo(msg, pr_name, pr_des, pr_count, pr_price):
    ...
    bot.send_message(admin_id, 'Товар успешно добавлен! Желаете что-то ещё?',
                     reply_markup=bt.admin_menu())
    # Переход на этап получения выбора
    bot.register_next_step_handler(msg, admin_choice)


# Этап подтверждения удаления
def confirm_delete(msg):
    admin_id = admin_id = 6775701667
    pr_name = msg.text
